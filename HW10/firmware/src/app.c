...
#define ARRLEN 14
#define MAFLEN 5//# of MAF variables
...
short Az=0;
float FIRWeight[MAFLEN]={0.05,0.05,0.1,0.3,0.5};
float IIRWeight[] = {0.3,0.7};
int MAFArr[MAFLEN];
int sum_MAF, avg_MAF, prev_IIR,avg_IIR,avg_FIR = 0;
...
void APP_Initialize(void) {
    ...
    Az = dtrans[6];
    for(j=0;j<MAFLEN-1;j++){
        MAFArr[j]=Az;//initilize the MAFArr
    }
    prev_IIR = Az;
        ...
        case APP_STATE_SCHEDULE_WRITE:
            ...
            if (appData.isReadComplete || flag) {
                ...
                for (i = 0; i <= 100; i++)
                {
                    ...
                    Az = dtrans[6];
                    MAFArr[MAFLEN-1] = Az;
                    sum_MAF = sum_MAF + MAFArr[MAFLEN-1];
                    avg_FIR = avg_FIR + MAFArr[MAFLEN-1]*FIRWeight[MAFLEN-1];
                    for(j=MAFLEN-2;j>=0;j++){
                        MAFArr[j]=MAFArr[j+1];//move and overwrite previous values
                        sum_MAF = sum_MAF + MAFArr[j];
                        avg_FIR = avg_FIR + MAFArr[j]*FIRWeight[j];
                    }
                    avg_MAF = sum_MAF/MAFLEN;
                    avg_IIR = prev_IIR*IIRWeight[0] + Az*IIRWeight[1];

                    len = sprintf(dataOut, "%d\t%d\t%d\t%d\t%d\r\n",i,(int)Az,avg_MAF,avg_IIR,avg_FIR);
